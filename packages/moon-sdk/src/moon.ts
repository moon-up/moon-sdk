...
import { Chains, IMoonSDK, MoonSDKConfig } from './types';
 ...
 *
 * @method setupAuthListeners - Set up authentication state change listeners.
 *
...
export class MoonSDK extends EventEmitter implements IMoonSDK {
  ...
  private erc4337: ERC4337Service;
  ...
  /**
   * Sets up authentication listeners to handle changes in authentication state.
   *
   * This method listens for authentication state changes such as signing in,
   * signing out, and token refresh events. Depending on the event, it updates
   * the authentication status, sets the security data, and emits corresponding
   * events.
   *
   * Events handled:
   * - 'SIGNED_IN': User has signed in.
   * - 'TOKEN_REFRESHED': User's authentication token has been refreshed.
   * - 'SIGNED_OUT': User has signed out.
   *
   * Actions performed:
   * - Updates the `isAuthenticated` status.
   * - Sets the security data in `moonAPIService`.
   * - Emits 'connected' or 'disconnected' events via `eventService`.
   */
