...
import type { MoonSDK } from "../moon"; // Assuming MoonSDK is in a separate file
...
export class AuthService {
  ...
  constructor ( moonSDK: MoonSDK ) {
    ...
    // initialise state change listener
  ...
  public async connect(
    ...
    try {
      ...
      // If no session exists and tokens are provided, set the session
      ...
      // Set the security data using the access token from the session
      this.moonSDK.moonAPIService.setSecurityData( session.access_token );
  ...
  async completePasskeyLogin(
    ...
    try {
      ...
      if ( !response.ok )
        throw new Error( `HTTP error! status: ${ response.status }` );
      const data = await response.json();
      await this.connect( data.token.access_token, data.token.refresh_token );
  ...
  onAuthStateChange(): () => void {
    const {
      ...
      if (
        ...
        this.isAuthenticated = true;
        await this.connect( session?.access_token, session?.refresh_token );
