...
export const useAuth = () => {
	const { moon, connect: MoonConnect } = useMoonAuth();
	 ...
	 * @returns {Promise<Session>} A promise that resolves when the connection is established.
	 */
	const connect = useCallback(
		async (accessToken?: string, refreshToken?: string): Promise<Session> => {
			return await moon.auth.connect(accessToken, refreshToken);
		},
		[moon],
	);
	...
	const signUpWithEmail = useCallback(
		async (email: string, password: string): Promise<Session | null> => {
			const session = await moon.auth.signUpWithEmail(email, password);
			return session;
		},
		[moon],
	);
	...
	const completePasskeyLogin = useCallback(
		async (
			...
			await MoonConnect(session.access_token, session.refresh_token);
