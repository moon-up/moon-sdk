...
export const useWalletTokenHistoryScanner = () => {
    ...
    const { data: ERC20Transfers, isFetching: fetchingERC20Transactions, error: ERC20Transactions } = useQuery({
        queryKey: ['ERC20TransfersHistory', walletAddressIn, chain?.chainId || ''],
        queryFn: async () => {
          if (!walletAddressIn || !chain?.chainId || !chain.blockExplorer || walletAddress == "") {
            console.log("useWalletTokens:debug transactions no data", walletAddressIn, chain?.chainId)
            return []
          }
          console.log("useWalletTokens:debug transactions", walletAddressIn, chain?.chainId)
          let tx = await localForage.getItem(txERC20CacheKey) as BlockExplorerERC20Transfer[] | null;
          if (!tx) {
            tx = await transactionHistoryManager.getERC20TransfersForAddressOnChain(walletAddress, chain.chainId)
